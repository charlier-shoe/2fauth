...
mcs._Utils.isEquivalentURL = function(url1, url2){

  if(url1.indexOf("https") === 0 && url2.indexOf("https") === 0){
    url1 = mcs._Utils.getPort(url1) === 443 ? url1.replace(':443', '') : url1;
    url2 = mcs._Utils.getPort(url2) === 443 ? url2.replace(':443', '') : url2;
  } else if(url1.indexOf("https") === -1 && url2.indexOf("https") === -1) {
    url1 = mcs._Utils.getPort(url1) === 80 ? url1.replace(':80', '') : url1;
    url2 = mcs._Utils.getPort(url2) === 80 ? url2.replace(':80', '') : url2;
  }
...
mcs._Utils.getPort = function(url){
  ...
  var BrowserPlatform = function() {
    ...
    this.invokeService = function (request) {
      ...
      if(request.success || request.error){
        invokeDeprecated(request);
      } else {
        // TODO: add validation for all urls, if the url is processed by Persistence library
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4) {
            if (xhr.status >= 200 && xhr.status <= 299) {
              ...
              var headers = xhr.responseHeaders || Persistence.Utils.parseResponseHeaders(xhr.getAllResponseHeaders());

              resolve({status: xhr.status, headers: headers, data: response});
            }
            else {
              reject(new mcs.ErrorResponse(xhr.status, xhr.response, xhr.getAllResponseHeaders()));
            }
          }
        };
       ...
       * @param request
       */
      function invokeDeprecated (request) {
        var xhr = new XMLHttpRequest();
        xhr.open(request.method, request.url);
        ...
        xhr.onreadystatechange = function() {
          if (xhr.readyState === 4) {
            var response = xhr.responseType && (xhr.responseType == '' || xhr.responseType == 'text') ? xhr.responseText : xhr.response;
            if (this.status >= 200 && this.status <= 299) {
              if (request.success != null) {
                var headers = xhr.responseHeaders || _parseResponseHeaders(xhr.getAllResponseHeaders());
                var jsonResponse = response;
                if(typeof response == "string"){
                  jsonResponse = response == "" ? {} : JSON.parse(response);
                }
                request.success(xhr, jsonResponse, headers);
              }
            }
            else{
              if(request.error != null){
                request.error(xhr.status, response)
              }
            }
          }
        };
...
}((typeof window != 'undefined' ? window.mcs : global.mcs) || {}));
...
function OAuthAuthorization(config, backend, appKey) {
  ...
  this.authenticateAnonymous = function (successCallback, errorCallback) {
    ...
    mcs.MobileBackendManager.platform.invokeService({
      ...
      success: function (response, data) {
        ...
        tokenExpiredTime = Date.now() + data.expires_in * 1000;
  ...
  this.logout = function() {
    ...
    tokenExpiredTime = Date.now() * 1000;
  };
...
(function(mcs) {
      ...
      if(method in httpMethods) {
        ...
        mcs.MobileBackendManager.platform.invokeService({
          ...
          success: function (response, data, headers) {
            if (successCallback != null) {
              successCallback(response.status, data, headers);
            }
          },
          error: function (statusCode, data) {
            if (errorCallback != null) {
  ...
  mcs._MobileBackend = MobileBackend;
...
(function(mcs) {
  ...
  var AUTHORIZATION_TYPES = { basicAuth: 'basicAuth', oAuth: 'oAuth', facebookAuth: 'facebookAuth', ssoAuth: 'ssoAuth'};
  ...
  function _initPersistenceConfiguration(config){
    if(!config.synchronization){
      mcs._Logger.log(mcs.logLevelError, 'WARNING, missing synchronization configuration, switch caching off');
      Persistence.Options.off = true;
      return;
    } else {
      Persistence.Options.off = false;
    }
    ...
    var persistenceConfig = {
      ...
      periodicRefreshInterval: config.synchronization.backgroundRefreshPolicy || 120,
      policies: []
    };

    var mcsPolicies = config.synchronization.policies;

    for(var prop in mcsPolicies){
      if(mcsPolicies.hasOwnProperty(prop)){
        var policy = mcsPolicies[prop];
        if(policy) {
          persistenceConfig.policies.push(getPersistencePolicy(prop, policy));
        } else {
          mcs._Logger.log(mcs.logLevelError, 'WARNING, the ' + policy + 'policy was not found in accepted policies.');
        }
      }
    }
    Persistence.Options.Policies = persistenceConfig;

    // default persistence library settings
    Persistence.Options.dbFirst = false;
    Persistence.Options.Module = Persistence.MCS;

    // fast fix for 404 bug
    Persistence.Options.maxSyncAttempts = 1;
    Persistence.Options.autoRemoveAfterReachMaxAttemps = true;
    // Persistence.Options.syncTimeOut = 3000;
    // Persistence.Options.autoSync = true;

    // TODO: for next release, add database prefix per user
    //Persistence.Options.dbPrefix = 'lpelov';
  }

  function getPersistencePolicy(path, mcsPolicy){
    var policy = {};
    policy.path = path;
    for(var prop in mcsPolicy){
      if(mcsPolicy.hasOwnProperty(prop)){
        var persMap = POLICIES_MAP[prop];
        policy[persMap.persistencePropertyName] = persMap[mcsPolicy[prop]]
      }
      if(policy[persMap.persistencePropertyName] === undefined){
        mcs._Logger.log(mcs.logLevelError, 'WARNING, the ' + prop + ' policy value ' + mcsPolicy[prop] + ' was not found in accepted policy values.');
      }
    }
    return policy;
  }
